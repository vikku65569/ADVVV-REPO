...
# Ask Doubt on telegram @SL_Necromancer
        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b><i>Searching For Movies 🔍</i></b>")
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"<b><i>Searching For movies 🔍</i></b>", reply_to_message_id=message.id)
     ...
     #   if not search:
      ...
      #      return
        ...
        try:
            ...
            return
    # don't change anything without contacting me @SL_Necromancer
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/SL_Necromancer")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
